...
export const PROCESSING_TIMEOUT_MS = 45_000;
...
export async function withProcessingTimeout<T>(work: Promise<T>): Promise<T> {
  ...
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(() => {
      reject(
        new Error(
          `Processing took longer than ${Math.round(PROCESSING_TIMEOUT_MS / 1000)} seconds and was stopped automatically. This can happen with a large file or high AI demand — try again, or use a smaller/clearer file.`
        )
      );
