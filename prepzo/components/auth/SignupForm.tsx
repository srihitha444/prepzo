...
function SignupFormContent({
  brandName = "Prepzo",
  tagline = "Create your free account — no credit card needed.",
}: SignupFormProps) {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        data: { full_name: name },
        emailRedirectTo: `${appUrl}${callbackPath}?next=${encodeURIComponent(redirectTo || "/dashboard")}`,
      },
    });
    if (error) {
    ...
    router.push(`${loginPath}?redirect=${encodeURIComponent(redirectTo || "/dashboard")}`);
    router.refresh();
  }
    ...
    <div className="min-h-screen bg-[#F8FAFF] flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <p className="text-center text-sm text-[#64748B] mt-6">
          ...
          <Link href={loginPath} className="text-[#1E3A8A] font-semibold hover:underline">
            Sign in
          </Link>
        </p>
      </div>
