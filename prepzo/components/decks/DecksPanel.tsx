...
import {
  ...
  ArrowLeft, BookOpen, Brain, ChevronLeft, Zap,
} from "lucide-react";
...
function getDeckSpeedSeconds(deck: "recall" | "review") {
  if (typeof window === "undefined") return deck === "recall" ? 5 : 7;
  try {
    const prefs = JSON.parse(localStorage.getItem("prepzo_prefs") || "{}");
    const key = deck === "recall" ? "recallSpeedModeInterval" : "reviewSpeedModeInterval";
    const fallback = deck === "recall" ? prefs.speedModeInterval || 5 : prefs.speedModeInterval || 7;
    return Number(prefs[key] || fallback);
  } catch {
    return deck === "recall" ? 5 : 7;
  }
}
...
export function DecksPanel() {
  ...
  const speedLockRef = useRef(false);
  ...
  async function loadItems() {
    ...
    const { data: mcqRaw } = await supabase
      ...
      .select(`id, times_seen, times_correct, next_due_at,
        questions!inner(id, question_text, subject, topic, difficulty, option_a, option_b, option_c, option_d, correct_option, explanation, exam)`)
      .eq("user_id", profile.id).eq("deck_type", activeTab).eq("questions.exam", profile.exam)
    ...
    const { data: fcRaw } = await supabase
      ...
      .select(`id, times_seen, next_due_at, flashcards!inner(id, front_text, back_text, subject, topic, exam)`)
      .eq("user_id", profile.id).eq("deck_type", activeTab).eq("flashcards.exam", profile.exam)
      .order("next_due_at", { ascending: true }).limit(50);
  ...
  async function handleMCQAnswer(option: string) {
    ...
    const isCorrect = isCorrectOption(option, item.correct_option);
  ...
  useEffect(() => {
    clearSpeedTimer();
    if (!reviewing || !speedMode || profile?.plan !== "paid") return;
    const item = items[reviewIndex];
    if (!item || item.kind !== "flashcard") return;

    const delay = Math.max(2, getDeckSpeedSeconds(activeTab)) * 1000;
    speedTimerRef.current = setTimeout(() => {
      if (!flipped) {
        setFlipped(true);
        return;
      }
      if (speedLockRef.current) return;
      speedLockRef.current = true;
      void handleFCMark(activeTab);
    }, delay);

    return clearSpeedTimer;
  }, [activeTab, clearSpeedTimer, flipped, items, profile?.plan, reviewIndex, reviewing, speedMode]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  if (reviewing) {
    ...
    const speedEligible = profile?.plan === "paid" && item.kind === "flashcard";
    const speedSeconds = getDeckSpeedSeconds(activeTab);
          ...
          <div className="flex items-center gap-3 text-sm">
            {speedEligible && (
              <button
                ...
                title={`Auto every ${speedSeconds}s`}
              >
                <Zap size={13} />
                {speedMode ? "Speed ON" : "Speed Mode"}
              </button>
            )}
          ...
          <>
            ...
            <div className="space-y-2.5 mb-4">
              {(["A", "B", "C", "D"] as const).map((label) => {
                ...
                const isCorrect = answered && isCorrectOption(label, item.correct_option);
                const isWrong = answered && selectedOption === label && !isCorrectOption(label, item.correct_option);
                return (
